...
function View(model)
{
 ...
 function init()
 {
	...
	wrapper=wrapper();
 	...
 	self.elements={
 		...
 		listContainer:$('.list'),
 		list:$('.list li')
 		//selectedElement:$('.list-item:nth-of-type('+(model.indexOfActive+1)+')')
 	};
 ...
 self.renderSelectedItem=function(indexOfActive){
   ...
   // self.elements.list
   $('.list li').css({"background-color": "lightskyblue"});
   // debugger
 	$('.list li:nth-of-type('+(+indexOfActive+1)+')').css({"background-color": "deepskyblue"});
 };
